...
import { foodMenu, ratingScores, ratingTags, shopDetails } from "../../service";
...
export default class Shop extends React.Component<any, any> {
  ...
  initCategoryNum = () => {
    ...
    this.state.menuList.forEach((item: any, index: number) => {
      if (
        ...
        let num = 0;
        Object.keys(this.state.shopCart[item.foods[0].category_id]).forEach(
          (itemid) => {
            Object.keys(
              this.state.shopCart[item.foods[0].category_id][itemid]
            ).forEach((foodid) => {
              let foodItem = this.state.shopCart[item.foods[0].category_id][
                itemid
              ][foodid];
  ...
  listenScroll = (element: any) => {
    ...
    this.state.foodScroll.on("scroll", (pos: any) => {
      ...
      this.state.shopListTop.forEach((item: any, index: number) => {
        ...
        <section className="shop_container">
          ...
          <section className="food_container">
            <section className="menu_container">
              ...
              >
                <ul>
                  {this.state.menuList.map((item: any, index: number) => {
                    return (
                      <li
                        key={index}
                        className={`menu_left_li ${
                          index === this.state.menuIndex ? "activity_menu" : ""
                          }`}
                        onClick={this.chooseMenu.bind(this, index)}
                      >
                        {item.icon_url
                          ? <img src={this.getImgPath(item.icon_url)} />
                          :
                          ""}
                        <span>{item.name}</span>
                        {this.state.categoryNum[index] && item.type === 1 ? (
                      ...
                      <li key={index}>
                        <header className="menu_detail_header">
                          ...
                          <span
                            ...
                            onClick={this.showTitleDetail.bind(this, index)}
                          ></span>
                        ...
                        {item.foods.map((foods: any, foodindex: number) => {
                          return (
                            <section
                              key={foodindex}
                            ...
                            >
                              <div className="menu_detail_link">
                                <section className="menu_food_img">
                                  <img
                                    src={
                                      this.state.imgBaseUrl + foods.image_path
                                    }
                                  />
                                </section>
                              ...
                              <footer className="menu_detail_footer">
                                ...
                                <Buycar
                                  ...
                                  showReduceTip={this.showReduceTip}
                                  showMoveDot={this.showMoveDotFun}
                                >
                                  {" "}
                                </Buycar>
                              </footer>
            ...
            <section className="buy_cart_container">
              ...
              >
                <div
                  className={`cart_icon_container ${
                    this.state.totalPrice > 0 ? "cart_icon_activity" : ""
                    } ${this.state.receiveInCart ? "move_in_cart" : ""}`}
              ...
              <section
                className={`gotopay ${
                  this.minimumOrderAmount <= 0 ? "gotopay_acitvity" : ""
                  }`}
              >
              ...
              {this.state.showCartList && this.state.cartFoodList.length ? (
                <section className="cart_food_list">
                  ...
                  <section className="cart_food_details" id="cartFood">
                    <ul>
                      {this.state.cartFoodList.map((item: any, index: any) => {
                        return (
            ...
            <div className="specs_list">
              ...
              <section className="specs_details">
                <h5 className="specs_details_title">
                  {this.state.choosedFoods.specifications[0].name}
                </h5>
                <ul>
                    ...
                    (item: any, index: number) => {
                      return (
                        <li
                          ...
                          onClick={this.chooseSpecs.bind(this, index)}
                        >
                          {item}
                        </li>
                      );
              ...
              <footer className="specs_footer">
                <div className="specs_price">
                  ...
                  <span>
                    {
                      this.state.choosedFoods.specfoods[this.state.specsIndex]
                        .price
                    }
                  </span>
                </div>
                <div
                  ...
                  onClick={this.addSpecs.bind(
                    ...
                    this.state.choosedFoods.specfoods[this.state.specsIndex]
                      .price,
                    this.state.choosedFoods.specifications[0].values[
